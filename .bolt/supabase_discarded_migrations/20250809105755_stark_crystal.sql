/*
  # Fix event attendees table with proper authentication

  1. New Tables
    - Drop and recreate `event_attendees` table with proper structure
    - `id` (uuid, primary key)
    - `event_id` (bigint, references events.id)
    - `user_id` (uuid, references users.id from existing schema)
    - `created_at` (timestamp)
    - `updated_at` (timestamp)

  2. Security
    - Enable RLS on `event_attendees` table
    - Add policy for public read access to view attendee counts
    - Add policy for authenticated users to manage their own attendance
    - Add unique constraint to prevent duplicate attendance

  3. Indexes
    - Add indexes for performance on event_id, user_id, and created_at
    - Add unique index for event_id + user_id combination
...
CREATE UNIQUE INDEX IF NOT EXISTS event_attendees_event_user_unique ON public.event_attendees(event_id, user_id);
